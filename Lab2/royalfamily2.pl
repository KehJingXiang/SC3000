...
all_descendants(Monarch, List):-
        monarch(elizabeth),
	findall(Child, offspring(Monarch, Child), List).
...
successor(Monarch, Succession_order):-
	all_descendants(Monarch, Children),
        succession_order(Children, Ordered_children),
	append(Ordered_children, [], Succession_order).
